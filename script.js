document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProducts() {
        try {
            ...
            // Do not display all products initially, show a prompt
                        resultsTableBody.innerHTML = `<tr><td colspan="8" class="text-start">검색어를 입력하고 검색 버튼을 누르세요.<br>유니테크 홈페이지 주소 : <a href="https://preview--unit-tech-automation.lovable.app/" target="_blank">https://preview--unit-tech-automation.lovable.app/</a></td></tr>`;
        } catch (error) {
    ...
    function displayProducts(products) {
        ...
        products.forEach(product => {
            ...
            row.innerHTML = `
                <td data-label="품목코드">${product['품목코드'] || ''}</td>
                <td data-label="품목명">${product['품목명'] || ''}</td>
                <td data-label="가격" class="price-normal">${product['가격'] || ''}</td>
                <td data-label="5%" class="price-5">${product['5%'] || ''}</td>
                <td data-label="7%" class="price-7">${product['7%'] || ''}</td>
                <td data-label="10%" class="price-10">${product['10%'] || ''}</td>
                <td data-label="13%" class="price-13">${product['13%'] || ''}</td>
